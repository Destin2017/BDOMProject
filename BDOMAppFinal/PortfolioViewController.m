...
@synthesize txtClientName = client_name;
@synthesize txtClientMobile = clientmobile_no;
@synthesize txtService = service;
@synthesize dpBook = date;
...
- (IBAction)btnBook:(id)sender {
    if([client_name.text isEqualToString:@""]
       ||[clientmobile_no.text isEqualToString:@""]
         ||[currentOrder.mobile_no isEqualToString:@""]
       ||[service.text isEqualToString:@""]
       ){
...
-(void) book {
    ...
    @try {
        ...
        NSString *post =[[NSString alloc] initWithFormat:@"client_name=%@&clientmobile_no=%@&service=%@&",[self.txtClientName text],[self.txtClientMobile text],[self.txtService text]];
        NSLog(@"PostData: %@",post);
        
        NSURL *url=[NSURL URLWithString:@"http://www.acknotech.co.ke/destinIOS/book.php"];
